...
display' chunks = let foo = S.each chunks :: St.ParallelT IO ([Vertex2 GLfloat]) -- O(n)
                      bar = (\vs -> do renderPrimitive Points $ mapM_ drawVert vs
...
display = do clear [ ColorBuffer ]
             displayCallback $= (do
                                    ...
                                    --display' c
                                    St.runStreamT $ St.liftIO (display' x) St.<|> St.liftIO (display' y) St.<|> St.liftIO (display' z)
                                )
